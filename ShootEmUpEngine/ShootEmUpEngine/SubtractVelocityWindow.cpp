...
SubtractVelocityWindow::SubtractVelocityWindow(int xLoc, int yLoc, int mainWindowWidth, int mainWindowHeight, EXEventComponent* myEvent, EXActionComponent* myAction, TTF_Font* inputFont)
{
	...
	gameWindow = SDL_CreateWindow("Subtract Velocity Window", xLoc + (mainWindowWidth / 2) - (windowWidth / 2), yLoc + (mainWindowHeight / 2) - (windowHeight / 2), windowWidth, windowHeight, SDL_WINDOW_BORDERLESS);
...
bool SubtractVelocityWindow::Update()
{
	int mouseX, mouseY;
	...
	{
		((EXAddVelocity*)pAction)->xVel = (std::stoi(velocityXTextBox->GetText()));
		((EXAddVelocity*)pAction)->yVel = (std::stoi(velocityYTextBox->GetText()));
		selectedVal = 1;
	}
...
void SubtractVelocityWindow::Draw()
{
...
int SubtractVelocityWindow::CheckSelected()
{
	return selectedVal;
}
